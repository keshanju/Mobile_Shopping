  ...
  <div id="app">
    ...
    <van-row gutter="20" class="row-1">
      ...
      <van-col span="16" class="cols">
        <form action="/">
          <van-search class="search" style="padding: 0;border-radius:22px;" v-model="value" placeholder="大家在搜索：衣服" @click.stop="search"></van-search>
        </form>
      </van-col>
      <van-col span="4" class="cols">
    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh" class="cartBox">
      <div v-for="(v,index) in shops" :key="index.id" class="shop-cart">
        ...
        <div @click="redirects('/goods/id_0/buy')">
          <van-card :title="goodsTitle[index].slice(0,16)" :desc="goodsDescription[0]" num="2" :price="prices[index]" :thumb="imageUrl[index]"></van-card>
        </div>
      </div>
    </van-pull-refresh>
    <!-- 优惠券列表 -->
