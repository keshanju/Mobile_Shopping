  ...
  <div id="app">
    <van-row class="col-me">
      ...
      <van-col span="24" class="headerList">
        <div class="me-img-ts">
          <span @click="redirects('/login')">{{login[0]}}</span>
        </div>
        <div class="me-img">
      ...
      <van-col span="24" class="boxer">
        <van-circle class="range" v-model="currentRate" :rate="94" :speed="100" :text="text" size="150px" color="#d34ba8" layer-color="#eee"></van-circle>
        <span>今日运势</span>
      </van-col>
      <!-- 弹出层 -->
      <van-popup v-model="shows" position="top" :overlay="true">
        <van-col span='24' class="title" style="width:400px;height:100%;paddingBottom:20px;">
          ...
          <div v-if="wuliu">
            <h4>{{menu[1]}}</h4>
            <van-steps :active="active" icon="logistics" title="" description="">
          ...
          <div v-if="contact" style="height:300px;">
            <h4>{{menu[2]}}</h4>
            <!-- 联系人卡片 -->
            <van-contact-card :type="cardType" :name="currentContact.name" :tel="currentContact.tel" @click="showList = true"></van-contact-card>
            <!-- 联系人列表 -->
            <van-popup v-model="showList" position="bottom">
              <van-contact-list v-model="chosenContactId" :list="list" @add="onAdd" @edit="onEdit" @select="onSelect"></van-contact-list>
            </van-popup>
            <!-- 联系人编辑 -->
            <van-popup v-model="showEdit" position="bottom">
              <van-contact-edit :contact-info="editingContact" :is-edit="isEdit" @save="onSave" @delete="onDelete"></van-contact-edit>
            </van-popup>
          ...
          <div v-if="pass" style="height:700px;">
            <h4>{{menu[3]}}</h4>
            <!-- 密码输入框 -->
            <van-password-input :value="value" info="密码为 6 位数字" @focus="showKeyboard = true"></van-password-input>
            <!-- 数字键盘 -->
            <van-number-keyboard :show="showKeyboard" @input="onInputs" @delete="onDelete" @blur="showKeyboard = false"></van-number-keyboard>
