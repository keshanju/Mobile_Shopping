<template>
	...
	<div id="app">
		<van-row class="col-me">
			...
			<van-popup v-model="shows" position="top" :overlay="true">
				<van-col
					span="24"
					class="title"
					style="width:400px;height:100%;paddingBottom:20px;"
				>
					<!-- 信用 -->
					<div v-if="xinyong">
						<h4>{{ menu[0] }}</h4>
					...
					<div v-if="wuliu">
						...
						>
							<van-step>
								<h3>
									【常州市】快件已到菜鸟驿站请带好证件来取件
								</h3>
								<p>2018-06-12 12:40</p>
							</van-step>
							<van-step>
								<h3>
									【无锡市】已到无锡集散中心正准备发往下一站
								</h3>
					...
					<div v-if="contact" style="height:300px;">
						...
						<div
							style="fontSize:12px;color:#A9A9A9;marginTop:20px;"
						>
							{{
								"保存联系人联系方式 能够有效的与他人取得联系哦~~"
							}}
						</div>
					</div>
					<!-- 密码设置 -->
		...
		<van-tabbar v-model="tabarActive">
			<van-tabbar-item icon="home-o" @click="redirects('/')"
				>首页</van-tabbar-item
			>
			<van-tabbar-item
				icon="friends-o"
				dot
				@click="redirects('/community')"
				>社区</van-tabbar-item
			>
			<van-tabbar-item
				icon="cart-o"
